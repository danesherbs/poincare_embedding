...
class PoincareNumpy(PoincareBase):
    ...
    def __init__(self, num_iter=10, num_negs=10, lr1=0.2, lr2=0.01,
        ...
        self.pembs = [
            np.random.uniform(low=-0.001, high=0.001, size=(self.dim,)) for _
            in range(len(self.pdict))]
    ...
    def backward(self, gdo, env):
        c = gdo;
        u, v, uu, uv, vv, alpha, beta, gamma = env
        ...
        for epoch in range(self.num_iter):
            ...
            for w1, w2 in pdata:
                ...
                for i, (i1, i2, d, env) in enumerate(exp_neg_dists):
                    ...
                    if gl is not None: pembs[i1] = self.add_clip(-lr,
                                                                 pembs[i1], gl)
                    if gr is not None: pembs[i2] = self.add_clip(-lr,
                                                                 pembs[i2], gr)
